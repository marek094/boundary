...
class CNN(nn.Module):
    def __init__(self, c=64, num_classes=10):
        ...
        self.conv0 = nn.Conv2d(3, c, kernel_size=3, stride=1, padding=1, bias=True)
        self.bn0 = nn.BatchNorm2d(c)
        # Layer 1
        self.conv1 = nn.Conv2d(c, c*2, kernel_size=3, stride=1, padding=1, bias=True)
        self.bn1 = nn.BatchNorm2d(c*2)
        # Layer 2
        self.conv2 = nn.Conv2d(c*2, c*4, kernel_size=3, stride=1, padding=1, bias=True)
        self.bn2 = nn.BatchNorm2d(c*4)
        # Layer 3
        self.conv3 = nn.Conv2d(c*4, c*8, kernel_size=3, stride=1, padding=1, bias=True)
