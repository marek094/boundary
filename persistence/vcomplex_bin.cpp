...
constexpr float overthreshold = 1e6;
...
template<typename T>
void bin_write_array(ostream& os, T* out, size_t size) {
    os.write(reinterpret_cast<char*>(out), size*sizeof(T));
}

template<typename T>
void bin_write(ostream& os, T out) {
    os.write(reinterpret_cast<char*>(&out), sizeof(T));
}
...
void compute(space_t& space) {
    for (int i=0; i < space.size(); i++) {
        ...
        for (int j=0; j<i; j++) {
            ...
            if (clss_a == clss_b) {
                bin_write(cout, overthreshold);
            } else {
                bin_write(cout, dist(feat_a, feat_b));
            }
        }
    }
}
